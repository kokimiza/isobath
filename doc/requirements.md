...
5. 次にログインしたとき、自分の観測番号・参加人数・海図形成の進捗を見る。現在地はまだ表示されない。

**S2. 暫定海図の上で現在地を知る（PROTO / SEED期）**

1. 初回測深を完了する。
2. 日次更新の後、次にログインしたときに、暫定射影上の現在地と推定精度を見る。SEED以降は、海域と所属確率も見る。
...
- **FR-STG-02 (MUST)**：段階の移行とモデルの改訂は自動では行わない。運営者が再計算の結果をレビューして承認し、**承認後の最初の日次更新（§4.1）の時点で有効になる**。日中に画面上の版・段階が変わらないこと。
...
**海図・現在地・参加人数は、回答した時点では更新しない。毎日 01:00（日本時間）に開始する日次バッチで一括して更新する。** 利用者は、次にログインしたときに更新後の結果を確認する。
...
| ID | 要求 | レベル |
|---|---|---|
| FR-BAT-01 | 海図（密度・等値線・集計値）、利用者の現在地、参加人数は、日次バッチでのみ更新すること。回答・完了の時点では更新しない | MUST |
...
| ID | 要求 | レベル |
|---|---|---|
| FR-POS-01 | PROTO以降、直近の日次更新で推定した現在地を、海図の座標系上に表示すること | MUST |
...
| FR-POS-07 | 最終更新日時を表示すること。まだ反映されていない完了済みの回答がある場合は、その旨と次回の更新予定時刻を表示すること | MUST |
...
| FR-OPS-02 | モデルの再計算（EFA・PCA・GMM 等）はローカルまたはCIのPythonスクリプトで実行し、レビュー用の成果物を出力すること。日次バッチはモデルを再計算しない | MUST |
| FR-OPS-03 | 承認したモデルと段階は、リポジトリへのマージで配置し、次の日次バッチの時点で有効になること | MUST |
...
| ID | 要求 | レベル |
|---|---|---|
| ST-POS-01 | 現在地は、日次バッチの中で、その時点で有効なモデルへの射影だけで算出すること。APIのリクエスト処理では算出しない。再学習もしない | MUST |
...
| GET | `/v1/meta` | サービスの状態、海図段階・版・参加人数（直近の日次更新の値）、最終更新日時、次回の更新予定時刻、Emergency Level | < 1 KB |
...
| GET | `/v1/me/position` | 直近の日次更新での現在地、推定精度、海域、所属確率、段階、更新日時、未反映の回答の有無 | < 2 KB |
...
日次更新に関する項目（すべてのレスポンスに含める）：

    {
      ...
      "pending": true                            // 締め時刻以降に完了した、未反映のセッションがある
    }
...
| batch_runs | 日次バッチの実行記録（締め時刻、版、段階、参加人数、生成した海図、成否）。締め時刻で一意 | API は SELECT のみ（集計値のため） |
