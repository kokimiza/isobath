...
> 2026-09-23追加：登録フローと研究専用属性の更新には `20260923010000_research_demographics.sql`、API、Web、プライバシー/研究説明書の版2をまとめて反映する。新規認証アカウントは未完了登録となるため、旧API/Webのまま新スキーマだけを運用しない。出生年月・性別はAuthメタデータには設定しない。Googleとメール確認の戻り先は従来どおり `/auth/callback`（英語は `/en/auth/callback`）。登録用の事前情報画面は `/auth/signup`、認証手段選択は `/auth/register`。旧版の研究同意者は再同意まで研究抽出・密度集計から外れる。施行日等の既存プレースホルダーは公開前に確定する。
