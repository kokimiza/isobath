...
def _place_users(conn, model: artifact.Model, cutoff: datetime, users: list) -> int:
    if not users:
        return 0
    answers: dict = {}
...
def _research_points(conn) -> np.ndarray:
...
def run(dsn: str, model: artifact.Model, now: datetime, k: int) -> dict:
    ...
    with psycopg.connect(
        ...
        try:
            with conn.transaction():
                ...
                if model.can_place:
                    placed = _place_users(
                        conn, model, cutoff, _targets(conn, cutoff, previous, model.version)
                    )
                    chart = density_map(_research_points(conn), k)
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run(dsn, artifact.load(s.models_dir), now, s.chart_k)
